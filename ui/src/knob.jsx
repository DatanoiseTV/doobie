...
function Knob({ value = 0.5, onChange, size = 'md', label, format, lit = false, bipolar = false, mod = 0, modValue = 0, arcColor, defaultValue }){
  ...
  const onDown = useCallback((e) => {
    ...
    const move = (ev) => {
      ...
      // 240 px per full knob range — one comfortable vertical mouse sweep
      // covers 0 -> 1 on a 13" MacBook without lifting / repositioning.
      // Shift drops to 1/4 speed (= 60 px / range = ~16-bit-feel) for the
      // fine adjustments that log-skewed params (delay TIME, filter
      // cutoff, etc.) want in their upper region.
